...
$sql = $conn->prepare("INSERT INTO school_map(map) VALUES (?)");
                $sql->execute([$file]);
                echo "<script>alert('MAP ADDED')</script>";
...
$sql = $conn->prepare("UPDATE school_map SET map = ? WHERE id = ?");
                $sql->execute([$file, $img_id]);
                echo "<script>alert('MAP UPDATED')</script>";
...
if(ISSET($_POST['delete'])){
    ...
    $stmt = $conn->prepare("DELETE FROM school_map WHERE id= ?");
    $stmt->execute([$id]);
    echo "<script>alert('MAP DELETED')</script>";
  ...
  <body class="u-body u-xl-mode" data-lang="en">
    <section class="u-clearfix u-gradient u-section-1" id="sec-f243">
      <div class="u-clearfix u-sheet u-sheet-1">
        ...
        <button type="button" class="myButton" data-toggle="modal" data-target="#myModal">ADD SCHOOL MAP</button>

        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModaldelete" style="float: right; margin-right: 5px;">DELETE MAP</button><br><br>
              ...
              <div class="u-container-layout u-similar-container u-container-layout-1">
                <div class="u-container-style u-custom-color-2 u-expanded-width-md u-expanded-width-sm u-expanded-width-xs u-group u-radius-30 u-shape-round u-group-1">
                  <div class="u-container-layout u-container-layout-2">
                    ...
                    <br><button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModalupdate_<?=$map['id']?>" style="float: right;">Update</button><br>
                  </div>
                </div>
              </div>
            </div>
...
<!---------------------------------------------UPDATE MAP MODAL----------------------------------------------------->
    <div class="modal fade" id="myModalupdate_<?=$map['id']?>">
   <div class="modal-dialog ">
       <div class="modal-content">
           <div class="modal-header">
              <h4 class="modal-title">UPDATE MAP</h4>
          ...
          <input type="hidden" class="form-control" value="<?=$map['id']?>" name="mapID" required>
   ...
   <div class="modal-dialog ">
       <div class="modal-content">
          ...
          <form action="" method="POST" enctype="multipart/form-data">
           ...
           <button name="add_map" class="btn btn-primary">+ ADD</button>
       ...
       <div class="modal-content">
          ...
          <form action="" method="POST" enctype="multipart/form-data">
                   ...
                   <div class="form-group">
                        ...
                        <select id="inputSchoolAdmin" name="map_iD" class="form-control">
                              <option></option>
                              <?php

                        $stmt = $conn->prepare("SELECT * FROM school_map");
                    $stmt->execute();
                     while ($SchoolMap = $stmt->fetch(PDO::FETCH_ASSOC)) {
                     echo '<option value="'.$SchoolMap['id'].'">School Map '.$SchoolMap['id'].'</option>';
                     }
                                ?>
                        </select>
                   </div>
